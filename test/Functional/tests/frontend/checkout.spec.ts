...
test.describe('Custom fees in checkout', (): void => {
    ...
    [
        {
            asCustomer: false,
            inEuro: false,
            testSuffix: 'for a guest',
        },
        {
            asCustomer: false,
            inEuro: true,
            testSuffix: 'for a guest, in Euro',
        },
        {
            asCustomer: true,
            inEuro: false,
            testSuffix: 'for a customer',
        },
        {
            asCustomer: true,
            inEuro: true,
            testSuffix: 'for a customer, in Euro',
        },
         ...
         */
        test(
            `Custom fees display in checkout ${testSuffix}`,
            ...
            async ({ page, browserName }): Promise<void> => {
                ...
                if (asCustomer) {
                    await test.step('Log in with account', async (): Promise<void> => {
                        ...
                        await page.goto(slugs.checkout.checkoutSlug);
                        /* TODO: select shipping method and payment method, then click on "Proceed ..."/"Next" button
                                 to go to payment step */
                    });
                } else {
                    /* TODO: Fill out address fields, select shipping method and payment method, and click on
                             "Proceed ..."/"Next" button to go to payment step */
                }
                ...
                await checkoutPage.hasCustomFees(inEuro, excludedFees);
            }
        );
    });
});
